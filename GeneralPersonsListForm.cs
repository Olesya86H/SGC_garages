...
{
    public partial class GeneralPersonsListForm : Form
    {
        ...
        public GeneralPersonsListForm()
        {
            InitializeComponent();           
        }

        private void GeneralPersonsListForm_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "sPersonsDS.s_persons". При необходимости она может быть перемещена или удалена.
            s_personsTableAdapter.Fill(sPersonsDS.s_persons);
        ...
        private void get_data()
        {
            ...
            dt.Rows.Add();
            cnt = dt.Rows.Count;
            FIOListgrd.CommitEdit(DataGridViewDataErrorContexts.Commit); // применяем изменения
            if (FIOListgrd.Rows.Count > 0)
            {               
                dt.Rows[cnt - 1].SetField(dt.Columns[0], FIOListgrd.Rows[row_inx].Cells[0].Value.ToString()); //ФИО
                dt.Rows[cnt - 1].SetField(dt.Columns[1], FIOListgrd.Rows[row_inx].Cells[1].Value.ToString()); //Телефон
                dt.Rows[cnt - 1].SetField(dt.Columns[2], FIOListgrd.Rows[row_inx].Cells[2].Value.ToString()); //Адрес
                dt.Rows[cnt - 1].SetField(dt.Columns[3], FIOListgrd.Rows[row_inx].Cells[3].Value.ToString()); //Примечания
                dt.Rows[cnt - 1].SetField(dt.Columns[4], FIOListgrd.Rows[row_inx].Cells[4].Value.ToString()); //Собственник
                dt.Rows[cnt - 1].SetField(dt.Columns[5], FIOListgrd.Rows[row_inx].Cells[5].Value.ToString()); //Плательщик
                dt.Rows[cnt - 1].SetField(dt.Columns[6], FIOListgrd.Rows[row_inx].Cells[6].Value.ToString()); //Пользователь ПС
                dt.Rows[cnt - 1].SetField(dt.Columns[7], FIOListgrd.Rows[row_inx].Cells[7].Value.ToString()); //ID
        ...
        {
            закрытьToolStripMenuItem2_Click(this, null);
        }

        private void добавитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            AddPersonForm addPersonForm = new AddPersonForm();
            addPersonForm.senderName = "GeneralPersonsListForm";
            addPersonForm.Show();
        }
        ...
        private void удалитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.action = "remove";
            get_data();
            FIOListgrd.Rows.RemoveAt(row_inx);
        }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    if (dt.Rows[i].ItemArray[8].ToString() == "remove")
                    {
                        ...
                        {
                            userClass.log_info("error - data not deleted from s_persons", "id_person = " + personClass.id_person.ToString());
                    ...
                    if (dt.Rows[i].ItemArray[8].ToString() == "upd")
                    {
                        ...
                        {
                            userClass.log_info("error - data not updated in s_persons", "id_person = " + personClass.id_person.ToString());
                ...
                s_personsTableAdapter.Fill(sPersonsDS.s_persons);
                FIOListgrd.DataSource = sPersonsDS.s_persons;
                FIOListgrd.Refresh();
            }
        }
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            formGenFind.senderName = "GeneralPersonsListForm";
            formGenFind.Show();   
        }

        private void FIOListgrd_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void FIOListgrd_RowLeave(object sender, DataGridViewCellEventArgs e)
        {
            //при добавлении или обновлении записи, когда строка потеряла фокус - перешли на др.строку - записываем данные из неё в dt
            if (e != null)
            {
                row_inx = e.RowIndex;
                col_inx = e.ColumnIndex;
        ...
        private void FIOListgrd_Leave(object sender, EventArgs e)
        {
            if (this.action == "upd")
                get_data();
        }

        private void FIOListgrd_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            row_inx = e.RowIndex;
            col_inx = e.ColumnIndex;
            DataGridViewCell cell = FIOListgrd.Rows[row_inx].Cells[col_inx];
            FIOListgrd.CurrentCell = cell;
            this.action = "upd";
        }
        ...
        {
            обновитьToolStripMenuItem_Click(this, null);
        }
    }
}
