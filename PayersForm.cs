...
{
    public partial class PayersForm : Form
    {
        ...
        private void PayersForm_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "s_payersDS.s_persons". При необходимости она может быть перемещена или удалена.
            s_payersTableAdapter.Fill(s_payersDS.s_persons);
        ...
        private void get_data()
        {
            ...
            dt.Rows.Add();
            cnt = dt.Rows.Count;
            if (FIOPayersgrd.Rows.Count > 0)
            {
                FIOPayersgrd.CommitEdit(DataGridViewDataErrorContexts.Commit); // применяем изменения
                dt.Rows[cnt - 1].SetField(dt.Columns[0], FIOPayersgrd.Rows[row_inx].Cells[0].Value.ToString()); //ФИО
                dt.Rows[cnt - 1].SetField(dt.Columns[1], FIOPayersgrd.Rows[row_inx].Cells[1].Value.ToString()); //Телефон
                dt.Rows[cnt - 1].SetField(dt.Columns[2], FIOPayersgrd.Rows[row_inx].Cells[2].Value.ToString()); //Адрес
                dt.Rows[cnt - 1].SetField(dt.Columns[3], FIOPayersgrd.Rows[row_inx].Cells[3].Value.ToString()); //Примечания
                dt.Rows[cnt - 1].SetField(dt.Columns[4], FIOPayersgrd.Rows[row_inx].Cells[4].Value.ToString()); //Собственник
                dt.Rows[cnt - 1].SetField(dt.Columns[5], FIOPayersgrd.Rows[row_inx].Cells[5].Value.ToString()); //Плательщик
                dt.Rows[cnt - 1].SetField(dt.Columns[6], "1"); //Пользователь ПС
                dt.Rows[cnt - 1].SetField(dt.Columns[7], FIOPayersgrd.Rows[row_inx].Cells[7].Value.ToString()); //ID
        ...
        {
            закрытьToolStripMenuItem_Click(this, null);
        }
        ...
        private void добавитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            addPersonForm.senderName = "PayersForm";
            addPersonForm.Show();
        }
        ...
        private void удалитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.action = "remove";
            get_data();
            FIOPayersgrd.Rows.RemoveAt(row_inx);
        }
        ...
        private void сохранитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                FIOPayersgrd_RowLeave(this, null);
            }
            catch { }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                s_payersTableAdapter.Fill(s_payersDS.s_persons);
                FIOPayersgrd.DataSource = s_payersDS.s_persons;
                FIOPayersgrd.Refresh();
            }
        }
        ...
        private void toolStripMenuItem2_Click(object sender, EventArgs e)
        {
            ...
            formGenFind.senderName = "PayersForm";
            formGenFind.Show();
        }
        ...
        private void удаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            удалитьToolStripMenuItem_Click(this, null);
        }

        private void FIOPayersgrd_RowLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (this.action == "upd")
                get_data();
        }

        private void FIOPayersgrd_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            row_inx = e.RowIndex;
            col_inx = e.ColumnIndex;
            DataGridViewCell cell = FIOPayersgrd.Rows[row_inx].Cells[col_inx];
            FIOPayersgrd.CurrentCell = cell;
            this.action = "upd";
        }

        private void FIOPayersgrd_Leave(object sender, EventArgs e)
        {
            if (this.action == "upd")
                get_data();
        }

        private void FIOPayersgrd_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
