...
namespace SGC_garages
{
    public partial class OwnersForm : Form
    {
        ...
        private void get_data()
        {
            ...
            dt.Rows.Add();
            cnt = dt.Rows.Count;
            FIOOwnersgrd.CommitEdit(DataGridViewDataErrorContexts.Commit); // применяем изменения
            if (FIOOwnersgrd.Rows.Count > 0)
            {
                dt.Rows[cnt - 1].SetField(dt.Columns[0], FIOOwnersgrd.Rows[row_inx].Cells[0].Value.ToString()); //ФИО
                dt.Rows[cnt - 1].SetField(dt.Columns[1], FIOOwnersgrd.Rows[row_inx].Cells[1].Value.ToString()); //Телефон
                dt.Rows[cnt - 1].SetField(dt.Columns[2], FIOOwnersgrd.Rows[row_inx].Cells[2].Value.ToString()); //Адрес
                dt.Rows[cnt - 1].SetField(dt.Columns[3], FIOOwnersgrd.Rows[row_inx].Cells[3].Value.ToString()); //Примечания
                dt.Rows[cnt - 1].SetField(dt.Columns[4], FIOOwnersgrd.Rows[row_inx].Cells[4].Value.ToString()); //Собственник
                dt.Rows[cnt - 1].SetField(dt.Columns[5], "1"); //Плательщик
                dt.Rows[cnt - 1].SetField(dt.Columns[6], FIOOwnersgrd.Rows[row_inx].Cells[6].Value.ToString()); //Пользователь ПС
                dt.Rows[cnt - 1].SetField(dt.Columns[7], FIOOwnersgrd.Rows[row_inx].Cells[7].Value.ToString()); //ID
        ...
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "s_OwnersDS.s_persons". При необходимости она может быть перемещена или удалена.
            s_OwnersTableAdapter.Fill(s_OwnersDS.s_persons);
        ...
        private void добавитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            addPersonForm.senderName = "OwnersForm";
            addPersonForm.Show();
        }
        ...
        private void удалитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            this.action = "remove";
            get_data();
            FIOOwnersgrd.Rows.RemoveAt(row_inx);
        }
        ...
        private void сохранитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                FIOOwnersgrd_RowLeave(this, null);
            }
            catch { }
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                s_OwnersTableAdapter.Fill(s_OwnersDS.s_persons);
                FIOOwnersgrd.DataSource = s_OwnersDS.s_persons;
                FIOOwnersgrd.Refresh();
            }
        }
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            formGenFind.senderName = "OwnersForm";
            formGenFind.Show();
        }
        ...
        private void FIOOwnersgrd_RowLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (this.action == "upd")
                get_data();
        }

        private void FIOOwnersgrd_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void FIOOwnersgrd_Leave(object sender, EventArgs e)
        {
            if (this.action == "upd")
                get_data();
        }

        private void FIOOwnersgrd_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            row_inx = e.RowIndex;
            col_inx = e.ColumnIndex;
            DataGridViewCell cell = FIOOwnersgrd.Rows[row_inx].Cells[col_inx];
            FIOOwnersgrd.CurrentCell = cell;
            this.action = "upd";
        }
    }
}
